// src/components/sections/Masonry.jsx
...
const Masonry = ({
  ...
  const grid = useMemo(() => {
    ...
    return items.map((child) => {
      ...
      // ✅ use the full height passed from items (no divide by 2)
  ...
  // decide where each card starts from (top, bottom, left, right, etc.)
  const getInitialPosition = (item, index) => {
    ...
    } else if (animateFrom === "alternateVertical") {
      // even index from top, odd index from bottom
  ...
  // --------- GSAP positioning / entrance animations -----------------------
  useLayoutEffect(() => {
      ...
      if (!hasMounted.current) {
        ...
        gsap.fromTo(
          ...
          initialState,
          {
            ...
            duration: 0.9, // slightly slower intro
            ease: "power3.out",
            delay: index * stagger
          }
        );
